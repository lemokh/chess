...
const possibleMoves = (passiveSide) => {
	...
	switch (pieceToMove.getAttribute('data-name')) {
		// -----------------------------------------
		case 'pawn':     pawnLit(passiveSide);     break;
		// -----------------------------------
...
const pieceLit = (e, passiveSide) => {
	...
	if (pinnedPieces.length) {
		...
		pinnedPieces.forEach(obj => {
			...
			// -------------------------------------------------------------
			else { possibleMoves(passiveSide); }
		});
	}
...
// window.onload = lit(blues, oranges);